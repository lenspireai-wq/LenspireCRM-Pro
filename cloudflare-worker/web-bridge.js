(() => {
  ...
  async function createBackup(payload){const password=String(payload?.password||'');if(password.length<12)throw new Error('A backup password of at least 12 characters is required.');emit('backup-progress',{phase:'backup',message:'Starting encrypted backup…',percent:20});const data=await api('/api/backup?password='+encodeURIComponent(password));emit('backup-progress',{phase:'backup',message:'Backup completed',percent:100});return{canceled:false,fileName:'lenspirecrm-backup.enc.json',encrypted:true,cloud:true,attachments:data?.attachments?.length||0,moduleCounts:data?.moduleCounts};}
  async function restoreBackup(){emit('backup-progress',{phase:'restore',message:'Select a backup file…',percent:10});const file=await chooseFile({accept:'.lenspirebackup,.json'});if(!file)return{canceled:true};emit('backup-progress',{phase:'restore',message:'Reading backup file…',percent:30});let backup;try{backup=JSON.parse(await file.text());}catch{throw new Error('Invalid backup file format.');}let password='';if(backup?.encrypted===true){emit('backup-progress',{phase:'restore',message:'Decrypting backup…',percent:50});password=window.prompt('Enter your backup password (at least 12 characters):');if(!password)throw new Error('Backup restore cancelled.');}emit('backup-progress',{phase:'restore',message:'Restoring to cloud…',percent:80});const result=await api('/api/backup/restore?password='+encodeURIComponent(password),{method:'POST',body:JSON.stringify(backup)});if(!result.ok)throw new Error(result.error||'Restore failed.');emit('backup-progress',{phase:'restore',message:'Restore complete',percent:100});return{workspace:await workspace(),compatibilityMode:false};}
  ...
  async function invoke(channel, ...args) {
    ...
    switch(channel){
      ...
      case 'restore-crm-backup': return restoreBackup(value);
      ...
      case 'get-autobackup-settings': {const data=await api('/api/backup/auto/config');const cfg=data?.config||{};const hours=cfg.intervalHours||24;const hh=String(Math.floor(hours)).padStart(2,'0');const mm=String(Math.round((hours-Math.floor(hours))*60)).padStart(2,'0');return{enabled:cfg.enabled??true,time:hh+':'+mm,password:''};}
      case 'set-autobackup-settings': {const settings=value||{};const parts=String(settings.time||'02:00').split(':');const intervalHours=Number(parts[0])+(Number(parts[1]||0)/60);return api('/api/backup/auto/config',{method:'POST',body:JSON.stringify({enabled:settings.enabled,intervalHours,retentionCount:7})});}
      case 'run-autobackup': return api('/api/backup/auto/trigger',{method:'POST',body:'{}'});

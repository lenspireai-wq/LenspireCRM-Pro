...
export default function ClientSetupPage(){
  ...
  return <main className="clientAuth"><form onSubmit={submit}><small>LENSPIRECRM · CLIENT PORTAL</small><h1>Create your password</h1><p>This invitation can be used only once.</p><label>New Password<input type={show?"text":"password"} minLength={4} required value={password} onChange={e=>setPassword(e.target.value)}/></label><label>Confirm Password<input type={show?"text":"password"} minLength={4} required value={confirm} onChange={e=>setConfirm(e.target.value)}/></label><label className="clientShowPassword"><input type="checkbox" checked={show} onChange={e=>setShow(e.target.checked)}/> Show password</label>{error&&<div className="error">{error}</div>}<button className="primary" disabled={busy}>{busy?"Creating…":"Create Password & Open Portal"}</button></form></main>
}

...
export default function ClientPortalPage() {
  ...
  return <main className="clientPortal">
    ...
    {selectedEvent && <div className="clientFeedbackBackdrop" role="dialog" aria-modal="true" aria-label="Event details"><section className="clientFeedbackModal clientEventModal"><small>READ-ONLY EVENT FORM</small><h2>{selectedEvent.event_type}</h2><dl><div><dt>Event date</dt><dd>{eventDate(selectedEvent)}</dd></div><div><dt>Time</dt><dd>{selectedEvent.start_time || "To be confirmed"}{selectedEvent.end_time ? ` – ${selectedEvent.end_time}` : ""}</dd></div><div><dt>Venue / city</dt><dd>{selectedEvent.city || "To be confirmed"}</dd></div><div><dt>Status</dt><dd>{selectedEvent.status}</dd></div></dl><div><button className="primary" onClick={() => setSelectedEvent(null)}>Close</button></div></section></div>}
  </main>;
}

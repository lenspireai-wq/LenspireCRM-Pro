...
export default function ClientPortalPage() {
  ...
  return <main className="clientPortal">
    ...
    {data.deliverables?.length ? <section className="clientPortalFaceSearch" aria-label="Find your wedding photos with LenspireAI">
      ...
      <button type="button" onClick={() => setFaceSearchNotice(true)}>Find My Photos <i>↗</i></button>
    </section> : null}
    ...
    {faceSearchNotice && <div className="clientFeedbackBackdrop" role="dialog" aria-modal="true" aria-label="Photo Finder access"><section className="clientFeedbackModal"><small>LENSPIREAI · PRIVATE SERVICE</small><h2>Your Photo Finder pass is not active yet</h2><p>AI face matching is enabled only when your studio activates it for this event. When it is ready, you will receive a private invitation to find your photos.</p><p>Your selfie and photos have not been uploaded or shared.</p><div><button className="primary" onClick={() => setFaceSearchNotice(false)}>Got it</button></div></section></div>}
  </main>;
}
